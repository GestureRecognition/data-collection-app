...
# Function to extract five frames from the middle 4/6 part of an MP4 file
def extract_frames(mp4_file, outfile_path):
    try:
        # Load the video clip
        clip = VideoFileClip(mp4_file)
        duration = clip.duration  # Get the total duration of the video

        # Calculate the middle 2/4 of the video (exclude 1/4 from front and back)
        start_time = duration / 4
        end_time = duration * 3 / 4
        middle_duration = end_time - start_time

        # Calculate the time interval to extract 5 equally spaced frames within the middle 4/6 portion
        intervals = [start_time + middle_duration * i / 4 for i in range(5)]
